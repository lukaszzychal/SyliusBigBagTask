...
Feature: Managing admin notes for orders
  ...
  Background:
    ...
    And there is a customer account "john.doe@example.com"
    And the store has a product "Test Product" with code "TEST-PRODUCT"
    And test order "000001" exists for "john.doe@example.com" with product "TEST-PRODUCT"
